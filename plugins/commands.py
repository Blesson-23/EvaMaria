...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        await message.reply_sticker(
            sticker="CAACAgQAAxkBAAEJuwhiuAxczaWIRVn85viiEogiC9nCGAAChgsAAmJWoFJF-tufwKSDax4E",
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await message.reply_sticker(
            sticker="CAACAgQAAxkBAAEJuw5iuAzcwvvunyAb7-vJkWhyD6fSBAACdAwAAiQ5AAFQ2rvUM6Wj_woeBA",
          #  caption=script.START_TXT.format(message.from_user.mention, temp.U_NAME, temp.B_NAME),
            reply_markup=reply_markup
            #parse_mode='html'
        )
...
@Client.on_message(filters.private & filters.command("tts"))
...
async def sng(bot, message):
        if not message.reply_to_message:
          ...
          try:
            await mee.delete()
            await bot.send_message(chat_id, text = rpl, reply_to_message_id = message.message_id)
          except Exception as e:                            
